...
# MetaX-specific GRU: the flagtree triton fork miscompiles several of this
# file's tl.dot patterns (pipelined fp32 dots produce wrong binaries for a
# large fraction of compiles, and fp64 dots return garbage deterministically),
# and its autotuner block-size shrink (AABS) lowers BLOCK_K below the dot's
# K >= 16 minimum when input_size <= 8. The dot-free FMA paths below avoid all
# three: fp64 always accumulates element-wise (NO_DOT), the input GEMM has a
# dedicated FMA kernel, and the fp32 recurrence uses the BLOCK_H=16 tile that
# the pipeliner compiles reliably (num_stages=1 everywhere).


def _fma_block_k(size: int, cap: int) -> int:
    # K chunk for the FMA fallbacks: sized to the actual reduction length so the
    # (BLOCK_B, BLOCK_K, BLOCK_N) intermediate stays register-resident, capped
    # because a few wide chunks amortize the load stream better than many small
    # ones (measured on C550: fp64 recurrence prefers 256, input GEMM 128).
...
def _transpose_weight_kernel(
    ...
    vals = tl.load(
        src + offs_r[:, None] * cols + offs_c[None, :], mask=mask, other=0.0
    )
...
def _pack_output_kernel(
    ...
    for f_block in range(0, tl.cdiv(hidden_size, BLOCK_F)):
        ...
        vals = tl.load(
            out_padded_ptr + t * out_stride_s + b * out_stride_b + offs_f * out_stride_f,
            mask=f_mask,
            other=0.0,
        )
        tl.store(
...
def _gru_input_gemm_fma_kernel(
    ...
    # Dot-free twin of _gru_input_gemm_kernel: same (seq, batch, gate) tile
    # precompute, but K is reduced with element-wise FMA instead of tl.dot.
    # Used on MetaX where tl.dot is unreliable (fp64 always, and fp32 when
    # input_size is too small for the dot's K >= 16 constraint under the
    # autotuner's block-size shrink). BLOCK_K is a small reduction chunk.
    pid_b = tl.program_id(0)
    seq_idx = tl.program_id(1)
    pid_n = tl.program_id(2)
    offs_b = pid_b * BLOCK_B + tl.arange(0, BLOCK_B)
    ...
    if PACKED:
        # Same early-out as the dot kernel: zero fully-inactive batch tiles.
        bs_t = tl.load(batch_sizes_ptr + seq_idx).to(tl.int32)
        if pid_b * BLOCK_B >= bs_t:
            out_offsets = (
                seq_idx * u_stride_s
...
def _gru_gemv_kernel(
    ...
    for step in range(seq_len):
        ...
        out_offsets = (
            t * out_stride_s
            + (out_feature_offset + offs_n) * out_stride_f
        )
...
def _run_direction(
    ...
    # fp64 tl.dot is broken, and for fp32 the autotuner's block-size shrink
    # lowers BLOCK_K below the dot's K >= 16 minimum when input_size <= 8.
    # Both cases take the dot-free FMA kernel with a fixed tile.
    use_fma_input = (
    ...
    with torch_device_fn.device(layer_input.device):
        if use_fma_input:
            # Tile tuned on C550 for fp64: a narrow batch tile and a wide K chunk
            # (register-resident (4, 128, 32) intermediate, 4 warps).
        ...
        # fp64 tl.dot is broken, so the recurrent GEMMs fall back to the
        # dot-free accumulation; fp32 keeps tl.dot but must not use the flaky
        # software pipeliner on the per-step kernel (see the module comment).
        ...
        if use_gemv:
            ...
            # Per-step launch. BLOCK_H=32 dots with pipelining are miscompiled by
            # the toolchain (BLOCK_H=16 is reliable), and the fp64 fallback wants
            # a narrow batch tile plus a wide K chunk (tuned on C550:
            # (4, 16, 256) at 8 warps).
            if no_dot_recur:
                ...
                # BLOCK_H=16 avoids the pipeliner miscompile; for very wide
                # hidden the doubled grid stops paying off (measured on C550),
                # and BLOCK_H=32 at num_stages=1 compiles reliably.
